...
class ProductOrders extends Round
{
    ...
    {
        $ylq=AnxjmBrand::where('id','<>',0)->count();
        $wcl=AnxjmBrand::where('state',-1)->count();
        $ywc=AnxjmBrand::where('state',1)->count();
        $feiqi=AnxjmBrand::where('state',0)->count();
        $jianzhi=Brand::where('web_anxjm',-9)->count();
     ...
     * @return $this
     */
    public function withContent($ylq, $wcl, $ywc,$feiqi,$jianzhi)
    {
        return $this->content(
            <<<HTML
<div class="col-12 d-flex flex-column flex-wrap text-center" style="max-width: 220px">
    ...
    <div class="chart-info d-flex justify-content-between mb-1">
          <div class="series-info d-flex align-items-center">
              <i class="fa fa-circle-o text-bold-700 text-gray"></i>
              <span class="text-bold-600 ml-50">兼职-分配</span>
          </div>
          <div class="product-result">
              <span>{$jianzhi}</span>
          </div>
