...
use App\Admin\Repositories\AnxjmBrand;
use App\Models\AnxjmBrand as AnxjmBrandModel;
...
class AnxjmBrandController extends AdminController
{
    /**
     * Make a grid builder.
     *
    ...
    {
        return Grid::make(new AnxjmBrand(), function (Grid $grid) {
            ...
            {
                $grid->column('untype')->using(['cf'=>'✤ 已发布','bcz'=>'☒ 不存在','wzl'=>'✎ 无资料','jzc'=>'✖ 禁止词','dyc'=>'☮ 地域词'])
                    /*->dot([
                        'default' => 'primary', // 设置默认颜色，不设置则默认为 default
                        'cf' => '#FF4040',
                        'bcz' => '#EE8262',
           ...
           if(admin_setting('keywords_wajue')==1 && Admin::user()->can('keywords_wajue'))
           {
            ...
            if (Admin::user()->isRole('administrator'))
            {
                $grid->filter(function (Grid\Filter $filter) {
                    $filter->panel();
                    $filter->equal('brand_userid','发布者')->select(AnxjmBrandModel::pluck('brand_username', 'brand_userid'))->width(3);
                });
            }
            //快速搜索
            ...
            $grid->actions(function (RowActions $actions) {
                    if($this->state==0)
                    {
                        $actions->append(new HuifuAction(AnxjmBrandModel::class));
                    }
                    elseif($this->state==-1)
                    {
                        $actions->append(new BrandOperationAction(AnxjmBrandModel::class));
                        $actions->append(new CompleteAction(AnxjmBrandModel::class));
                    }
    ...
    {
        return Show::make($id, new AnxjmBrand(), function (Show $show) {
            $show->field('id');
            $show->field('brand_id');
    ...
    {
        return Form::make(new AnxjmBrand(), function (Form $form) {
            $form->display('id');
            $form->text('brand_id');
